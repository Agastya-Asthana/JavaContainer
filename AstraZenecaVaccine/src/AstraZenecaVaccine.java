 ...
 * objects is optional. You do NOT need to follow the couse style guide or commenting requirements
 * for this exam.
 *
 */
...
public class AstraZenecaVaccine {
   ...
   * array. Recall that recipients array is NOT a bag. The order of elements stored in the
   * recipients array must be maintained after adding newRecipient to the array. This means that a
   * shift operation is needed. If there is no room in the recipients array to add the new
   * recipient, the message error "Warning! Full Array!" will be printed without making any change
   * to the contents of the array. To be eligible to take the covid-19 AstraZeneca vaccine and be
   * added to the list of recipients, the recipient must not be older than 55.
   * 
  ...
  public static int addRecipientToFront(String[] recipients, int size, String newRecipient,
    ...
    if (age > 55) throw new IllegalArgumentException("Recipient is not elligible to take vaccine");

    if (recipients.length > size) {
      for (int i = size; i >= 0; i--) {
        if (i == 0) recipients[i] = newRecipient;
        else recipients[i] = recipients[i-1];
      }
      size++;
    }
    else{
  ...
  public static int giveVaccineDoseToLast(String[] recipients, int size) {
    if (dosesCount == 0) throw new IllegalStateException("No doses remaining!");

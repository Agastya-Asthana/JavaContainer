...
public class Animal implements GUIListener {
  ...
  /**
   * This method returns whether or not the mouse is over the current animal
   * @return true if mouse is over the animal and false otherwise
   */
  @Override
  public boolean isMouseOver(){
    if (this.getX() == processing.mouseX && this.getY() == processing.mouseY){
      return true;
    }
    return false;
  }
  ...
  /**
   * Sets the isDragging varibale to true when the user presses down on this object
   */
  @Override
  ...
  /**
   * Sets the isDragging variable to false when the user lets go of this object
   */
  @Override
  ...
  */
  public boolean isClose(int x, int y, int range) {
    return (distance(x, y) >= 0 && distance(x, y) <= range) ? true : false;
  }
  ...
  */
  public boolean isClose(Animal otherAnimal, int range) {
    int x = otherAnimal.getX();
    int y = otherAnimal.getY();
    return (distance(x, y) >= 0 && distance(x, y) <= range) ? true : false;
  }
